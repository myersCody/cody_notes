...
class GCPReportDownloader(ReportDownloaderBase, DownloaderInterface):
    ...
    def get_manifest_context_for_date(self, date):
        ...
        manifest_dict = {}
        report_dict = {}
        invoice_month = date.strftime("%Y%m")
        assembly_id = self._generate_assembly_id(invoice_month)
        with ReportManifestDBAccessor() as manifest_accessor:
            manifest = manifest_accessor.get_manifest(assembly_id, self._provider_uuid)
        LOG.info(f"assembly_id: {assembly_id}")
        LOG.info(f"self._provider_uuid: {self._provider_uuid}")
        LOG.info(f"manifest: {manifest}")
        if manifest:
            LOG.info("MANIFEST FOUND!!!!!!")
            manifest_id = manifest.id
            # TODO: See if I need to populate this
            files_list = []
        else:
            manifest_dict = self._generate_monthly_pseudo_manifest(date, assembly_id)
            ...
            files_list = [
                {"key": key,
                "local_file": self.get_local_file_for_report(key)} for key in manifest_dict.get("file_names")
            ]
    ...
    def _generate_monthly_pseudo_manifest(self, start_date, assembly_id):
        """
        ...
        Args:
            ...
            assembly_id (str): see _generate_assembly_id()
    ...
    def _generate_assembly_id(self, invoice_month):
        ...
        """
        fake_assembly_id = ":".join([str(self._provider_uuid), str(invoice_month)])
